...
var window_width = 230; // width of window
var window_height = 135; // height of window
...
if (ie)
{
	...
	// We are creating a text range from the whole document body	 self.frames['myIframe'].
...
function findit() 
{
	...
	else // Netscape or firefox
	{
		...
		if (finds > 0)
		{
			sel = window.self.frames['myIframe'].getSelection(); // get selection
			// remove all ranges
		...
		if (string != "")	
		{
			if (window.self.frames['myIframe'].find(string, false, false))
			{
	...
	//document.getElementById('print_div').style.marginTop = '61px';
...
function findprev()
{
	...
	else // if netscape or firefox
	{
		...
		// window.find(string, caseSensitive, searchBackwards)
		if (string != "")
			test.innerHTML = (window.self.frames['myIframe'].find(string, false, true)?"<span style='color: #390; text-align: center; display: block'>Encontrado</span>":"<span style='color: #F00; text-align: center; display: block'>No Encontrado</span>");	
		// In Firefox (not Netscape) when we press the
			// Next or Prev buttons in the DIV it looses the
			// selection, so we have to grab the selection
			// locations so we don't keep searching for only
...
//	t = setInterval('move_window();', 500); 	
	...
	document.onkeydown = checkkey;
		...
		window.self.frames['myIframe'].find();
...
function create_div(dleft, dtop, dwidth, dheight)
{
    ...
    findwindow.style.top = "5%";
	findwindow.style.left = "45%";
	findwindow.style.padding = '0px'; 
	findwindow.style.zIndex = 9000;
	findwindow.style.fontSize = '14px';
	findwindow.style.overflowX = 'hidden';
	findwindow.className = 'mi_boxitis'
...
// This part creates the instructions and the "find" button
	findwindow.innerHTML += '<div id="window_body" style="padding: 10px;">'
	+ 'Escriba el texto: '
	+ '<input type="text" size="25" maxlength="25" id="fwtext"'
	+ ' onchange="resettext();">'
	+ '<input type="button" value="Atras" class="mi_buttoncito" onclick="findprev();">'
	+ '<input id="btn" type="button" value="Siguiente" class="mi_buttoncito" onclick="findit();">'
	+ '</div>\n'
	+ '<div id="test" style="padding-left: 5px;"><br /></div>';
...
//document.write('<input type="button" style="display: none;" value="Find on this page..."'
//	+ ' onclick="show();">');
