...
var window_border = "#01224f"; // the border color of pop-up window
...
var start_at = 0; // Change to which character you want to start with on the page if IE gives an 800a025e error because of searching in menus
...
// Following lines added 12/4/2013. Ver 2.3d
/*var isIE11 = !!navigator.userAgent.match(/Trident\/7\./);
if (isIE11)
    var ie = document.getElementsByTagName('*');
else
    var ie = (document.all);
*/
...
if (window.find)
{
	var ie = 0; // Added 6/16/2014 Ver 2.3e
	var nav = 1; // to detect if netscape or firefox
}
else 
{
	var ie = 1; // Added 6/16/2014 Ver 2.3e
	var nav = 0;
}
...
function findit() 
{
	...
	if (string)  // only call findText if there is a string or IE will have error
		...
		{	
			try // v. 2.3 - 7/23/2013 - Added try and catch for ie error 800a025e
			{
				// select() not only highlights the string but
				// it also moves the view to that location
				txt.select();
			}
			catch(err) // IE: Could not complete the operation due to error 800a025e
			{
				/* This error occurs only in IE if the search term is found in a display:none element,
					usually a menu system at the top of the webpage. This try and catch fixes the problem
					by not selecting the text but moving to the next point with txt.collapse(false); and 
					doing the search again with findit();  Interestingly, if the menu system used 
					visibility: hidden instead of display:none then IE just skips searching for the text
					in that hidden element instead of throwing the error.
				*/		
				test.innerHTML = "Found in display:none element";
				finds++;
				txt.collapse(false);
			...
			test.innerHTML = "<span style='color: #390; text-align: center; display: block'>Encontrado</span>";
		}
		else
		{
			test.innerHTML = "<span style='color: #F00; text-align: center; display: block'>No Encontrado</span>";
			finds--;
		}
	}
	else // Netscape or firefox
	{
		...
		if (string != "")	
		{
			if (window.find(string, false, false))
			{
				...
				sel = window.getSelection(); // get selection
				if(sel.rangeCount > 0) // v. 2.2 - 10/16/2012
...
function findprev()
{
	...
	else // if netscape or firefox
	{
		...
		// window.find(string, caseSensitive, searchBackwards)
		if (string != "")
			if (window.find(string, false, true))
				test.innerHTML = "<span style='color: #390; text-align: center; display: block'>Encontrado</span>";
			else
				test.innerHTML = "<span style='color: #F00; text-align: center; display: block'>No Encontrado</span>";	
		// In Firefox (not Netscape) when we press the
...
// This part creates the instructions and the "find" button
	...
	+ ' onchange="resettext();">'
	+ '<input type="button" value="atras" onclick="findprev();">'
