...
function HitTracker_construct(document_window)
{
	...
	this.current_match = HitTracker_getCurrentMatch(document_window);
	...
	this.pathname = document_window.location.pathname;
...
function HitTracker_checkState(document_window)
{
	...
	{
		if (document_window.location.pathname != this.pathname)
		{
...
function HitTracker_moveToAnchor(anchor)
{
	...
	{
		// Work around IE 5.5 bug -- changing anchor doesn't move document after XSL transformation.
		if (-1 != navigator.appVersion.indexOf("MSIE 5.5"))
		{
			for (var a = 0; a < this.document_window.document.anchors.length; a++)
			{
				if (this.document_window.document.anchors[a].name == anchor)
				{
					...
					this.document_window.scrollBy(-this.document_window.document.body.offsetWidth, 0);
					break;
				}
			}
		}
...
function HitTracker_getCurrentMatch(document_window)
{
	var current_match = HitTracker_getFirstMatch(document_window.document);
	
	var hash = document_window.location.hash;
	if (hash.length != 0 && hash.substring(1, hit_prefix.length + 1) == hit_prefix)
	{
		current_match = parseInt(hash.substring(hit_prefix.length + 1));
	}
