...
{
	if (!node)
		node =  window.self.frames['myIframe'].document.body; // document.body;
...
function findit() 
{
	...
	//	unhighlight(); // Remove highlights of any previous finds
		...
		{
		$("#find_msg").html(  "&nbsp;<b>0 de 0</b>"); // ver 5.1 - 10/17/2014 - changed from "Not Found"
			find_pointer = -1;	
		}
	}
	findwindow.style.visibility = 'visible';
...
function findnext()
{
	...
	$("#find_msg").html( display_find+" de "+highlights.length)
...
function hide()
{	
//	unhighlight(); // Remove highlights of any previous finds - ver 5.1 - 10/17/2014  // last change in my script for javascript 
	findwindow.style.visibility = 'hidden';
...
function create_div(dleft, dtop, dwidth, dheight)
{
    ...
    if (document.getElementById('findwindow'))
    {
        ...
        win_iframe = document.getElementById('myIframe');
    }
    else
    { 
...
//	+ ';float: left' 
	...
	+ 'Buscar palabra(s) <button type="button" id="btnbusque" class="btn btn-danger btn-sm" onclick="hide();"><i class="glyphicon glyphicon-remove"></i> </button></div>';
...
// This part creates the instructions and the "find" button
	findwindow.innerHTML += '<div id="window_body" class="panel-body" style="background-color: white; padding-bottom: 0px; padding-top: 8px;">'
	+'<div class="form-group">'
	+ '<label for="fwtext">Escriba el texto a buscar</label> '
	+ '<input type="text" size="25" class="form-control" id="fwtext" placeholder="escribe aqui.."' 
	+ ' onchange="resettext();"> </label></div>'	
...
//	+ '<input id="btn" type="button" value="Find Next" onclick="this.blur(); findit();">' // ver 5.3 - 5/15/2015 - added this.blur(); lal tecniadfjlsd fjsd sdjfds lla tecniacn es la mayor de todos los
//	+ ' <span id="find_msg"><br /></span>'
//	+ '</form></div>\n';
	+ '</div>\n'
	+ '<div id="" class="panel-footer text-center">'
	+ '<input type="button" value="◄" title="Anterior" class="btn btn-primary" onclick="findprev();"> '
	+ '<input id="btn" type="button" value="►" title="Siguiente" class="btn btn-primary" onclick="this.blur(); findit();">'
...
document.write('<input type="button" value="Find on this page..."' 	+ ' onclick="show();">');
